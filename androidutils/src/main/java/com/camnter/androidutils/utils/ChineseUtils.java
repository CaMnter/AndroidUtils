...
public class ChineseUtils {
    ...
    private static int[] spellingValue = new int[] { -20319, -20317, -20304, -20295, -20292, -20283,
    ...
    public static String[] spellingString = new String[] { "a", "ai", "an", "ang", "ao", "ba",
            "bai", "ban", "bang", "bao", "bei", "ben", "beng", "bi", "bian", "biao", "bie", "bin",
            ...
            "die", "ding", "diu", "dong", "dou", "du", "duan", "dui", "dun", "duo", "e", "en", "er",
     ...
     * @return ascii
     */
    private int getChineseAscii(String chinese) {
        int ascii = 0;
        try {
            byte[] bytes = chinese.getBytes("gb2312");
            if (bytes.length > 2 || bytes.length <= 0) {
                throw new RuntimeException(TAG + ": Illegal chinese string");
            }
            if (bytes.length == 1) {
                ascii = bytes[0];
            }
     ...
     * @return string
     */
    public String convert(String chinese) {
        String result = null;
        int ascii = getChineseAscii(chinese);
        if (ascii > 0 && ascii < 160) {
            result = String.valueOf((char) ascii);
        } else {
            for (int i = (spellingValue.length - 1); i >= 0; i--) {
                if (spellingValue[i] <= ascii) {
                    result = spellingString[i];
                    break;
                }
            }
        }
